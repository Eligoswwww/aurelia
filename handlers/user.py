...
from chatter.core import get_response, train_default, train_custom, reset_db, DB_PATH
import os
...
async def cmd_debug(message: types.Message):
    ...
    try:
        sample = get_response("Привет")
    except Exception as e:
        sample = f"Ошибка: {e}"
    await message.answer(f"📊 Статус:
- SQLite: {'✅ есть' if exists else '❌ нет'}
- Файл: `{db_file}`
- Ответ на \"Привет\": {sample}")
...
def register_handlers(dp: Dispatcher):
    ...
    dp.message.register(cmd_debug, Command(commands=["debug"]))
